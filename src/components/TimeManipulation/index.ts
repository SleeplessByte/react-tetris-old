...
export const AsyncTimeManipulation = Loadable({
  loader: () => new Promise((resolve) => {
    require.ensure([], function (require) {
      resolve(require<ModuleWithDefaultExport<any>>('./TimeManipulation').default)
    }, 'time-manipulation')
  }),
  LoadingComponent: Loading,
  webpackRequireWeakId: () => {
    return require.resolveWeak('./TimeManipulation')
  }
})

AsyncTimeManipulation.displayName = 'Async(TimeManipulation)'

export function preload() {
  AsyncTimeManipulation.preload()
}

export default AsyncTimeManipulation
